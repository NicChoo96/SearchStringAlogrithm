...
from python_src import fileWriteReadParser, algorithm_playground
...
def show_two_charts(file1, file2):
    ...
    for i in range(0, len(files)):
        genome_size = fileWriteReadParser.get_json_data(files[i])[0]['genome_length']
        df = DataFrame(fileWriteReadParser.get_chart_data(files[i]), columns=['Strain_Length', 'Comparison'])
        if i == 0:
...
def show_single_chart(file_name):
    json_data = fileWriteReadParser.get_json_data(file_name)
    if len(json_data) == 0:
        return
    genome_size = json_data[0]['genome_length']
    root = tk.Tk()
    root.title(file_name)
    df = DataFrame(fileWriteReadParser.get_chart_data(file_name), columns=['Strain_Length', 'Comparison'])
    figure = plt.Figure(figsize=(5, 4), dpi=100)
    ax = figure.add_subplot(111)
...
def main_gui():
    ...
    algorithm_playground.text_output = text
    algorithm_playground.root = root
    ...
    existing_files = fileWriteReadParser.get_list_output_files()
...
def generate_algorithm(text, g_length, dna_strain, listbox):
    ...
    if g_length == "":
        algorithm_playground.random_genome_length = 400000
    else:
        genome_length = int(g_length)
        if genome_length > 5000000:
            genome_length = 5000000
        algorithm_playground.random_genome_length = genome_length
    if dna_strain == "":
        algorithm_playground.main_test()
    else:
        algorithm_playground.single_test_run(dna_strain)
    if not fileWriteReadParser.check_output_exist():
        listbox.insert(tk.END, fileWriteReadParser.file_name)


def show_file(text, listbox):
    data = fileWriteReadParser.filter_file_data(listbox.get(tk.ANCHOR))
    text.configure(state=tk.NORMAL)
    text.delete('1.0', tk.END)
...
def select_algo(selector):
    algorithm_playground.algorithm_selector = selector


def toggle_genome():
    algorithm_playground.use_genome_file = not algorithm_playground.use_genome_file
