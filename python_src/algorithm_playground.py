...
from python_src import boyer_moore, brute_force, front_back, fileWriteReadParser, algorithm_data_formatter
...
def algorithmRun(genome, dnaStrain):
    if algorithm_selector == 0:
        brute_force.search(genome, dnaStrain)
    elif algorithm_selector == 1:
        front_back.search(genome, dnaStrain)
    elif algorithm_selector == 2:
        boyer_moore.search(genome, dnaStrain)
...
def run_genome_test(dnaStrain):
    ...
    algorithm_data_formatter.is_record_genome = False
...
def main_test():
    ...
    print_gui("Output File: " + fileWriteReadParser.file_name)


def single_test_run(dna_strain):
    print_gui(getAlgoName() + " single test run Started")
    if use_genome_file:
        fileWriteReadParser.clear_data_file(genome_run_file_single[algorithm_selector])
        fileWriteReadParser.file_name = genome_run_file_single[algorithm_selector]
        run_genome_test(dna_strain)
    else:
        fileWriteReadParser.clear_data_file(multi_run_file_single[algorithm_selector])
        fileWriteReadParser.file_name = multi_run_file_single[algorithm_selector]
        run_multiple_test(random_genome_length, dna_strain)
